...
async def list_site_trend(site_name: str, filter_date: date, session: AsyncSession = Depends(get_db)):
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to fetch {site_name} trend data: {str(e)}")
...
async def search_words_trend(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to fetch {q} trend data: {str(e)}")
